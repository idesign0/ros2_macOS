...
ROS2_SRC="$HOME/kilted-ros2/src"

echo "Extracting find_package names from ROS2 source..."
grep -rho 'find_package([^)]*)' "$ROS2_SRC" | sed 's/find_package(//; s/)//' | sort -u > $HOME/kilted-ros2/src/brew-packages/found_packages.txt
...
grep -Ff $HOME/kilted-ros2/src/brew-packages/found_packages.txt $HOME/kilted-ros2/src/brew-packages/brew_packages_versions.txt > $HOME/kilted-ros2/src/brew-packages/matched_packages.txt
echo "Matched packages saved to matched_packages.txt."

# Count matched lines
matched_count=$(wc -l < $HOME/kilted-ros2/src/brew-packages/matched_packages.txt)
